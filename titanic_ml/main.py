...
@click.option(
    "--output", type=click.Path(path_type=Path), help="Output path for predictions"
)
...
@click.option("--age", type=float, required=True, help="Age of passenger")
@click.option("--sibsp", type=int, default=0, help="Number of siblings/spouses aboard")
@click.option("--parch", type=int, default=0, help="Number of parents/children aboard")
@click.option("--fare", type=float, required=True, help="Passenger fare")
@click.option(
    "--embarked",
...
def _display_training_results(results: Dict[str, Any]) -> None:
    ...
    for model_name, model_results in training_results.items():
        ...
        table.add_row(model_name, f"{val_acc:.4f}", f"{cv_mean:.4f}", f"{cv_std:.4f}")
...
def _display_single_prediction(
    passenger_data: Dict[str, Any], result: Dict[str, Any]
) -> None:
...
def _display_data_analysis(df) -> None:
    ...
    if "Survived" in df.columns:
        ...
        for row in survival_by_class.iter_rows(named=True):
            console.print(
                f"  Class {row['Pclass']}: {row['survival_rate']:.3f} ({row['count']} passengers)"
            )
