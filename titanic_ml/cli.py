...
def train(
    data_path: Annotated[
        Path, typer.Option(help="Path to data directory containing train.csv")
    ] = Path("data"),
    model_type: Annotated[str, typer.Option(help="Type of model to train")] = "xgboost",
    output_dir: Annotated[
        Path, typer.Option(help="Directory to save trained model")
    ] = Path("models"),
    optimize: Annotated[
        bool,
        typer.Option(
            "--optimize/--no-optimize", help="Enable hyperparameter optimization"
        ),
    ] = True,
    trials: Annotated[int, typer.Option(help="Number of optimization trials")] = 100,
    cv_folds: Annotated[int, typer.Option(help="Number of cross-validation folds")] = 5,
    random_state: Annotated[
        int, typer.Option(help="Random state for reproducibility")
    ] = 42,
    verbose: Annotated[
    ...
    console.print(
        f"Hyperparameter optimization: {'Enabled' if optimize else 'Disabled'}"
    )
    ...
    try:
        with Progress(
            ...
            if "train" in data_info:
                ...
                console.print(
                    f"Training data: {info['rows']} rows, {info['columns']} columns, {info['memory_mb']} MB"
                )
...
def predict(
    ...
    data_path: Annotated[
        Path, typer.Option(help="Path to data directory containing test.csv")
    ] = Path("data"),
    output_file: Annotated[
        Optional[Path], typer.Option(help="Output file for predictions (CSV format)")
    ] = None,
    ...
    try:
        with Progress(
            ...
            if "test" in data_info:
                info = data_info["test"]
                console.print(
                    f"Test data: {info['rows']} rows, {info['columns']} columns, {info['memory_mb']} MB"
                )
            ...
            temp_data = test_data.with_columns(
                pl.lit(0).alias("Survived")
            )  # Add dummy target
            ...
            if probabilities:
                ...
                survival_probabilities = prediction_probabilities[
                    :, 1
        ...
        results = pl.DataFrame({"PassengerId": passenger_ids, "Survived": predictions})
...
@app.command()
def info(
    data_path: Annotated[Path, typer.Option(help="Path to data directory")] = Path(
        "data"
    ),
) -> None:
    ...
    try:
        ...
        for column in train_data.columns:
            ...
            columns_table.add_row(column, dtype, str(non_null), f"{null_pct:.1f}%")
        ...
        if "Survived" in train_data.columns:
            ...
            for row in survival_stats.rows():
                ...
                survival_table.add_row(
                    "Yes" if survived == 1 else "No", str(count), f"{percentage:.1f}%"
                )
