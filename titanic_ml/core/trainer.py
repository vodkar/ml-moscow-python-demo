...
class ModelTrainer:
    ...
    def train(
        ...
        if (
            hasattr(self.config, "enable_hyperparameter_tuning")
            and self.config.enable_hyperparameter_tuning
        ):
    ...
    def _create_model(self, params: Dict[str, Any]):
        ...
        base_params = {"random_state": self.config.random_state, "n_jobs": -1}
...
class EnsembleTrainer:
    ...
    def train(
        ...
        for i, config in enumerate(self.model_configs):
            logger.info(
                f"Training model {i + 1}/{len(self.model_configs)}: {config.model_type}"
            )
