...
class FeatureEngineer:
    ...
    def extract_title(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        return df.with_columns([
            pl.col("Name").map_elements(get_title, return_dtype=pl.Utf8).alias("Title")
        ])
    ...
    def create_family_features(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        return df.with_columns([
            ...
            pl.when(pl.col("FamilySize") == 1).then(pl.lit("Single"))
            .when(pl.col("FamilySize").is_between(2, 4)).then(pl.lit("Small"))
            .when(pl.col("FamilySize").is_between(5, 7)).then(pl.lit("Medium"))
            .otherwise(pl.lit("Large")).alias("FamilySizeCategory")
        ])
    ...
    def create_age_features(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df_with_age = df.with_columns([
            pl.col("Age").fill_null(
                pl.col("Age").median().over(["Title", "Pclass"])
            ).fill_null(pl.col("Age").median())
        ])
        
        return df_with_age.with_columns([
            # Age categories
            pl.when(pl.col("Age") <= 16).then(pl.lit("Child"))
            .when(pl.col("Age").is_between(17, 32)).then(pl.lit("Young_Adult"))
            .when(pl.col("Age").is_between(33, 48)).then(pl.lit("Adult"))
            .when(pl.col("Age").is_between(49, 64)).then(pl.lit("Middle_Aged"))
            .otherwise(pl.lit("Senior")).alias("AgeCategory"),
    ...
    def create_fare_features(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df_with_fare = df.with_columns([
            pl.col("Fare").fill_null(
                pl.col("Fare").median().over("Pclass")
        ...
        return df_with_fare.with_columns([
            ...
            .when(pl.col("Fare").is_between(7.92, 14.45)).then(pl.lit("Medium"))
            .when(pl.col("Fare").is_between(14.46, 31.0)).then(pl.lit("High"))
            .otherwise(pl.lit("Very_High")).alias("FareCategory")
        ])
    ...
    def create_cabin_features(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        return df.with_columns([
            ...
            # Cabin deck
            pl.col("Cabin").map_elements(get_cabin_deck, return_dtype=pl.Utf8).alias("CabinDeck")
        ])
    
    def encode_categorical_features(self, df: pl.DataFrame, is_training: bool = True) -> pl.DataFrame:
        """Encode categorical features."""
        console.print("Encoding categorical features...")
        
        categorical_columns = ["Sex", "Embarked", "Title", "FamilySizeCategory", 
    ...
    def select_features(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        feature_columns = [
            ...
            "FamilySizeCategory_encoded", "AgeCategory_encoded", "AgeBin",
            "FarePerPerson", "FareCategory_encoded", "HasCabin", "CabinDeck_encoded"
        ]
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        if not self.is_fitted:
            raise ValueError("FeatureEngineer must be fitted before transforming test data")
