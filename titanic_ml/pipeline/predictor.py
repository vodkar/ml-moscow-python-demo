...
class TitanicPredictor:
    ...
    def load_pipeline(
        self, model_path: Path, preprocessor_path: Optional[Path] = None
    ) -> None:
    ...
    def predict_batch(
        ...
        # Create results
        results = []
        for i, (passenger_id, pred, prob) in enumerate(
            zip(passenger_ids, predictions, probabilities)
        ):
    ...
    def create_submission_file(
        self, test_df: Union[pd.DataFrame, pl.DataFrame], output_path: Path
    ) -> None:
...
class ModelEvaluator:
    ...
    def analyze_feature_importance(
        self, trainer: ModelTrainer, feature_names: List[str], top_n: int = 10
    ) -> Dict[str, float]:
    ...
    def analyze_prediction_confidence(
        ...
        analysis = {**confidence_stats, **probability_stats, **confidence_distribution}
    ...
    def generate_report(self, output_path: Optional[Path] = None) -> Dict:
        ...
        report = {"timestamp": pd.Timestamp.now().isoformat(), "results": self.results}
