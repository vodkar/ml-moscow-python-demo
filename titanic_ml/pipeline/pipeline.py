...
class TitanicMLPipeline:
    ...
    def run_full_pipeline(
        ...
        if len(model_configs) == 1:
            ...
            model, metrics = self.trainer.train(
                x_train, y_train, self.preprocessor.get_feature_importance_names()
            )
            ...
            results["ensemble"] = {
                "models": [
                    {"model_type": config.model_type, "metrics": metrics.model_dump()}
    ...
    def _load_data(
        ...
        pd.DataFrame | pl.DataFrame,
        Optional[pd.DataFrame | pl.DataFrame],
    ]:
    ...
    def _save_pipeline(self) -> None:
        ...
        if isinstance(self.trainer, ModelTrainer):
            ...
            for i, trainer in enumerate(self.trainer.trainers):
                model_path = (
                    self.config.model_output_path / f"ensemble_model_{i}.joblib"
                )
    ...
    def get_model_summary(self) -> Dict:
        ...
        if isinstance(self.trainer, ModelTrainer):
            summary = {
                ...
                "metrics": self.trainer.training_metrics.model_dump()
                if self.trainer.training_metrics
                else None,
            }
        elif isinstance(self.trainer, EnsembleTrainer):
            summary = {
                ...
                "n_models": len(self.trainer.trainers),
                "model_types": [
                    trainer.config.model_type for trainer in self.trainer.trainers
                ],
                "weights": self.trainer.weights,
                "individual_metrics": [
                    trainer.training_metrics.model_dump()
