...
class PredictionPipeline:
    ...
    def predict_from_file(self, test_file_path: Path, output_path: Optional[Path] = None) -> Path:
    ...
    def predict_single(self, passenger_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {
            ...
            "survival_probability": float(survival_probability) if survival_probability is not None else None,
            "model_used": self.model_trainer.best_model_name
        }
        
        return result
    
    def batch_predict(self, passenger_list: list[Dict[str, Any]]) -> list[Dict[str, Any]]:
        ...
        for i, passenger_data in enumerate(passenger_list):
            result = {
                ...
                "survival_probability": float(survival_probabilities[i]) if survival_probabilities[i] is not None else None,
                "model_used": self.model_trainer.best_model_name
            }
