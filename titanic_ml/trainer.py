...
class TrainerConfig(BaseModel):
    ...
    optimization_trials: int = Field(
        default=100, description="Number of optimization trials"
    )
    model_save_path: Path = Field(
        default=Path("models"), description="Path to save trained models"
    )
...
class TitanicTrainer:
    ...
    def _create_base_model(self) -> Any:
        ...
        if self.config.model_type == "logistic_regression":
            ...
            return lgb.LGBMClassifier(
                random_state=self.config.random_state, verbosity=-1, force_row_wise=True
            )
    ...
    def _objective(
        self, trial: optuna.Trial, features: np.ndarray, target: np.ndarray
    ) -> float:
    ...
    def _prepare_features_and_target(
        ...
        feature_columns = [
            col for col in dataframe.columns if col not in ["Survived", "PassengerId"]
        ]
    ...
    def _evaluate_model(
        self, model: Any, features: np.ndarray, target: np.ndarray
    ) -> ModelPerformance:
