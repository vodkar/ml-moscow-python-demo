...
class DataLoaderConfig(BaseModel):
    ...
    lazy_loading: bool = Field(default=True, description="Use lazy loading for large datasets")
    chunk_size: int | None = Field(default=None, description="Chunk size for processing large files")
...
class TitanicDataLoader:
    ...
    def _validate_schema(self, dataframe: pl.DataFrame, required_columns: set[str]) -> None:
    ...
    def load_train_data(self, force_reload: bool = False) -> pl.DataFrame:
        ...
        logger.info(f"Loaded training data: {dataframe.shape[0]} rows, {dataframe.shape[1]} columns")
    ...
    def load_test_data(self, force_reload: bool = False) -> pl.DataFrame:
        ...
        logger.info(f"Loaded test data: {dataframe.shape[0]} rows, {dataframe.shape[1]} columns")
        logger.info(f"Memory usage: {dataframe.estimated_size('mb'):.2f} MB")
