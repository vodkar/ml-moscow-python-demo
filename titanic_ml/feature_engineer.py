...
FAMILY_SIZE_MAPPING: Final[dict[str, int]] = {
    "Alone": 0,
    "Small": 1, 
    "Large": 2
}
...
class FeatureEngineerConfig(BaseModel):
    ...
    fill_missing_embarked: bool = Field(default=True, description="Fill missing embarked values")
    fill_missing_fare: bool = Field(default=True, description="Fill missing fare values")
    create_title_feature: bool = Field(default=True, description="Extract title from name")
    create_family_features: bool = Field(default=True, description="Create family size features")
    ...
    create_cabin_features: bool = Field(default=True, description="Extract cabin deck and number")
    drop_original_features: bool = Field(default=True, description="Drop original features after transformation")
...
class TitanicFeatureEngineer:
    ...
    def _extract_title(self, dataframe: pl.DataFrame) -> pl.DataFrame:
        ...
        return dataframe.with_columns([
            ...
            .map_elements(lambda x: TITLE_MAPPING.get(x, "Rare"), return_dtype=pl.Utf8)
            .alias("Title")
        ])
    ...
    def _fill_missing_values(self, dataframe: pl.DataFrame, is_training: bool = True) -> pl.DataFrame:
        ...
        if self.config.fill_missing_embarked and "Embarked" in dataframe.columns:
            if is_training:
                embarked_mode = dataframe.select(pl.col("Embarked").mode().first()).item()
    ...
    def _encode_categorical_features(self, dataframe: pl.DataFrame) -> pl.DataFrame:
        ...
        if "Embarked" in dataframe.columns:
            for port in ["C", "Q", "S"]:
                expressions.append(
                    pl.when(pl.col("Embarked") == port).then(1).otherwise(0).alias(f"Embarked_{port}")
                )
        ...
        if "Title" in dataframe.columns:
            for title in ["Mr", "Miss", "Mrs", "Master", "Rare"]:
                expressions.append(
                    pl.when(pl.col("Title") == title).then(1).otherwise(0).alias(f"Title_{title}")
                )
        ...
        if "FamilySizeCategory" in dataframe.columns:
            for category in ["Alone", "Small", "Large"]:
                expressions.append(
                    pl.when(pl.col("FamilySizeCategory") == category).then(1).otherwise(0).alias(f"Family_{category}")
                )
        ...
        if "CabinDeck" in dataframe.columns:
            ...
            for deck in cabin_decks:
                expressions.append(
                    pl.when(pl.col("CabinDeck") == deck).then(1).otherwise(0).alias(f"Deck_{deck}")
                )
        ...
        if "AgeBand" in dataframe.columns:
            for band in AGE_BAND_LABELS:
                expressions.append(
                    pl.when(pl.col("AgeBand") == band).then(1).otherwise(0).alias(f"Age_{band}")
                )
        ...
        if "FareBand" in dataframe.columns:
            for band in FARE_BAND_LABELS:
                expressions.append(
                    pl.when(pl.col("FareBand") == band).then(1).otherwise(0).alias(f"Fare_{band}")
                )
    ...
    def get_feature_names(self, dataframe: pl.DataFrame) -> list[str]:
        ...
        feature_columns = [col for col in temp_df.columns if col not in ["Survived", "PassengerId"]]
